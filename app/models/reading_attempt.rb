class ReadingAttempt < ApplicationRecord
  ...
  validates :wpm, numericality: { greater_than: 0 }
  ...
  def next_target_wpm
    user_attempts = ReadingAttempt.where(user: user, reading_passage: reading_passage)
                                  .where("comprehension >= ?", 70)
    best = [user_attempts.maximum(:wpm).to_i, wpm].max
    (best * 1.1).round
  end
end
